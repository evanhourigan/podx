...
def md_to_blocks(md: str) -> List[Dict[str, Any]]:
    ...
    for raw in lines:
        ...
        if m:
            ...
            if level == 1:
                blocks.append({"object": "block", "type": "heading_1", "heading_1": {"rich_text": rt(text)}})
            elif level == 2:
                blocks.append({"object": "block", "type": "heading_2", "heading_2": {"rich_text": rt(text)}})
            else:
                blocks.append({"object": "block", "type": "heading_3", "heading_3": {"rich_text": rt(text)}})
            continue
        
        # Quote
        qm = re.match(r"^\s*>\s*(.+)$", line)
        if qm:
            blocks.append({"object": "block", "type": "quote", "quote": {"rich_text": rt(qm.group(1))}})
            continue
        
        # Bulleted list
        bm = re.match(r"^\s*[-*+]\s+(.+)$", line)
        if bm:
            blocks.append({"object": "block", "type": "bulleted_list_item", "bulleted_list_item": {"rich_text": rt(bm.group(1))}})
            continue
        
        # Numbered list
        nm = re.match(r"^\s*(\d+)\.\s+(.+)$", line)
        if nm:
            blocks.append({"object": "block", "type": "numbered_list_item", "numbered_list_item": {"rich_text": rt(nm.group(2))}})
            continue
        ...
        if not line.strip():
            ...
            blocks.append({"object": "block", "type": "paragraph", "paragraph": {"rich_text": list(chunk_rich_text(line))}})
...
@click.command()
@click.option("--db", "db_id", default=lambda: os.getenv("NOTION_DB_ID"), help="Target Notion database ID")
@click.option("--title", help="Page title (or derive from --meta)")
@click.option("--date", "date_iso", help="ISO date (YYYY-MM-DD) (or derive from --meta)")
@click.option("--markdown", "md_path", type=click.Path(exists=True, path_type=Path), required=True, help="Markdown file")
@click.option("--json", "json_path", type=click.Path(exists=True, path_type=Path), help="Structured JSON for extra Notion properties")
@click.option("--meta", "meta_path", type=click.Path(exists=True, path_type=Path), help="Episode metadata JSON (to derive title/date)")
@click.option("--title-prop", default=lambda: os.getenv("NOTION_TITLE_PROP", "Name"), help="Notion property name for title")
@click.option("--date-prop", default=lambda: os.getenv("NOTION_DATE_PROP", "Date"), help="Notion property name for date")
@click.option("--dry-run", is_flag=True, help="Parse and print Notion payload (don't write)")
def main(
    db_id: Optional[str],
    title: Optional[str],
    date_iso: Optional[str],
    ...
    if json_path:
        ...
        if key_points:
            props_extra["Tags"] = {
                "multi_select": [{"name": kp[:50]} for kp in key_points]  # Notion name limit
            }
