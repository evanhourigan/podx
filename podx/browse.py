...
class EpisodeBrowser:
    ...
    def display_page(self) -> None:
        ...
        for i, episode in enumerate(page_episodes):
            ...
            table.add_row(str(episode_num), date, duration, title)
...
def main(
    ...
    Browse episodes with pagination and select one. Episode metadata is automatically
    saved to a JSON file for easy processing with other podx commands.
    """
    ...
    if not output:
        # Auto-generate filename based on show and episode
        safe_show = "".join(
            c for c in (show or "podcast") if c.isalnum() or c in (" ", "-", "_")
        ).rstrip()
        safe_show = safe_show.replace(" ", "_").lower()
    ...
    if process:
        ...
        try:
            ...
            browser.console.print(
                f"❌ Episode processing failed with exit code {e.returncode}"
            )
            sys.exit(e.returncode)
    else:
        # Show summary instead of raw JSON
        browser.console.print(f"\n✅ Episode metadata saved to [cyan]{output}[/cyan]")
        browser.console.print(
            f'🎙️ Ready to process with: [yellow]podx run --show "{show}" --date {episode_date}[/yellow]'
        )
