...
@dataclass
class CloudConfig:
    """Configuration for RunPod cloud transcription.

    Attributes:
        api_key: RunPod API key (from RUNPOD_API_KEY env var)
        endpoint_id: RunPod serverless endpoint ID (from RUNPOD_ENDPOINT_ID env var)
        timeout_seconds: Maximum time to wait for transcription (default: 600s/10min)
    ...
    def is_configured(self) -> bool:
        """Check if cloud is configured (has API key and endpoint)."""
        return bool(self.api_key and self.endpoint_id)

    @property
    def base_url(self) -> str:
        """Get the RunPod API base URL for this endpoint."""
